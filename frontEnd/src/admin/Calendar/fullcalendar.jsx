...
const Calendar = () => {
  ...
  useEffect(() => {
    const filteredAppointments = fetchedAppointments.filter((event) => {
      const searchValue = searchTerm.trim().toLowerCase();
      return (
        (event.title?.toLowerCase() || "").includes(searchValue) ||
        (event.extendedProps.email?.toLowerCase() || "").includes(
          searchValue
        ) ||
        (event.extendedProps.contact?.toLowerCase() || "").includes(
          searchValue
        ) ||
        (event.extendedProps.consultationType?.toLowerCase() || "").includes(
          searchValue
        ) ||
        (event.extendedProps.additionalInfo?.toLowerCase() || "").includes(
          searchValue
        ) ||
        (event.extendedProps.platform?.toLowerCase() || "").includes(
          searchValue
        ) ||
  ...
  }, [searchTerm, fetchedAppointments]);
        ...
        <div className="content-fullcalendar">
          <Box m="0px">
            ...
            <div>
              ...
              >
                <MenuItem onClick={handleAppointmentClick}>
                  Appointment
                </MenuItem>
                    ...
                    {searchTerm.trim() ? (
                      currentEvents.length > 0 ? (
                        currentEvents.map((event) => (
                          <div className="sidebar-calendar">
                          <ListItem key={event.id} sx={{ mb: 1, paddingLeft: "5px" }}>
                            <ListItemText
                              primary={event.title}
                              secondary={
                                <><br />
                      ...
                      fetchedAppointments
                        ...
                        .map((event) => (
                          <ListItem key={event.id} sx={{ mb: 1, paddingLeft: "5px" }}>
                            {/* Render selected event details as before */}
                            <ListItemText
                              primary={event.title}
